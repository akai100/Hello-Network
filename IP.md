...
+ 版本字段（4 比特）
  ...
  该版本字段标识互联网头部的格式。本文档描述的是版本 4（即 IPv4）

+ 互联网头部长度（IHL）（4 字节）

  表示的 IP 头部总长度，通过该字段可定位数据包中数据部分的起始位置。注意，一个合法 IP 头部的 IHL 字段最小值为5（对应最小头部长度 20 字节）。

+ 服务类型字段（ToS）（8 比特）

  服务类型字段用于指明数据包期望的服务质量（QoS）相关抽象参数。当数据报通过特定网络传输时，这些参数将用于指导实际服务参数的选择。
  部分网络支持服务优先级机制 —— 该机制会将高优先级流量视为高于其他流量（通常在网络高负载时，仅接收优先级高于特定阈值的流量）。
  其核心是在低延迟、高可靠性、高吞吐量这三个指标之间做出三选一的权衡取舍‘

  第 0 - 2 位：优先级位段

  第 3 位：延迟标识位——0 = 普通延迟，1 = 低延迟
  
  第 4 位：吞吐量标识位——0 = 普通吞吐量，1 = 高吞吐量

  第 5 位：可靠性标识位——0 = 普通可靠性，1 = 高可靠性
...
```
 ...
 101 - 关键 / 紧急控制协议（CRITIC/ECP）

 100 - 紧急强占（Flash Override）
 ...
 启用延迟、吞吐量和可靠性相关标识（即设置对应比特位），可能会在某种意义上增加服务成本。在多数网络中，提升其中一项指标的性能，往往会导致另一项指标的性能下降。除极少数特殊情况外，这三项标识中最多只能设置两项。

 服务类型（ToS）字段用于指定数据报在互联网系统中传输期间所应获得的处理方式。关于互联网服务类型如何映射到特定网络（如自动数字网二代（AUTODIN II）、阿帕网（ARPANET）、卫星网（SATNET）和分组无线电网（PRNET））实际提供的服务，相关示例映射关系详见《服务映射》文档 [参考文献 8]。

 网络控制（Network Control）优先级标识仅适用于网络内部使用。该标识的具体使用方式与管控权限由各个网络自行决定。网间控制（Internetwork Control）优先级标识仅限网关控制发起方使用。若某一特定网络关注这些优先级标识的实际使用情况，则由该网络自行负责管控对这些优先级标识的访问权限及使用行为。

+ 总长度（Total Length）：16 比特

  总长度字段表示数据报的完整长度（以字节为单位），包含互联网头部（IP 头部）和数据部分。该字段允许数据报的最大长度为 65535 字节，但如此长的数据报在大多数主机和网络中并不实用。
  所有主机必须能够接收长度不超过 576 字节的数据报（无论数据报是完整到达还是分片到达）。建议主机仅在确认目的端能够接收更大尺寸数据报的情况下，才发送超过 576 字节的数据报。

  选择 576 字节这一数值，是为了在容纳必需的头部信息之外，还能传输一个合理大小的数据块。例如，该尺寸可使一个 512 字节的数据块与 64 字节的头部信息共同适配于一个数据报中。
  互联网头部的最大长度为 60 字节，而典型的互联网头部长度为 20 字节，这就为高层协议的头部预留了冗余空间。

+ 标识（Identification）：16 比特、

  标识字段是由发送方分配的一个标识值，用于协助重组数据报的分片。

+ 标志位（Flags）：3 比特

  用于承载各类控制标志，各比特位定义如下：

  + 第 0 位：保留位（Reserved），必须设为 0

  + 第 1 位：不分片标志（DF，Don’t Fragment）→ 0 = 允许分片（May Fragment），1 = 禁止分片

  + 第 2 位：更多分片标志（MF，More Fragments）→ 0 = 最后一个分片（Last Fragment），1 = 还有更多分片（More Fragments）
...
+ 分片偏移量（Fragment Offset）：13 比特

  该字段标识当前分片在原始数据报中的相对位置。

  分片偏移量以 8 字节（64 比特）为单位进行度量。第一个分片的偏移量为 0。

+ 生存时间（Time to Live，TTL）：8 比特

  该字段标识数据报在互联网系统中允许存在的最长时间。若此字段值为 0，则必须销毁该数据报。互联网头部处理过程中会修改该字段：其时间单位为秒，但由于每个处理数据报的网络设备（模块）即使在不到 1 秒内完成处理，
  也必须将 TTL 值至少减 1，因此 TTL 应仅被视为数据报可能存在的最长时间上限。设置该字段的目的是丢弃无法交付的数据报，并限制数据报的最大生存周期。

+ 协议（Protocol）：8 比特

  该字段标识互联网数据报数据部分所使用的下一层协议。各类协议对应的取值在《编号分配（Assigned Numbers）》[9] 中有明确规定。

+ 头部校验和（Header Checksum）：16 比特

  仅对 IP 头部进行校验的校验和字段。由于部分头部字段（如生存时间 TTL）会发生变化，因此在每一处处理互联网头部的节点上，都必须重新计算并验证该校验和。
  校验和算法定义如下：

  校验和字段的值是 “头部所有 16 比特字的反码和” 的 16 比特反码。计算校验和时，需将校验和字段自身的值预设为 0。

  该校验和算法计算简便，实验数据表明其能满足需求，但此算法为临时性方案，未来可能根据进一步的实践经验，被循环冗余校验（CRC）机制替代。
...
+ 选项
  ...
  选项字段（Options）可在数据报中选择性出现，但所有 IP 模块（主机与网关）都必须实现该字段的处理逻辑。其 “可选性” 体现在：是否在特定数据报中传输该字段，而非是否实现该字段的处理功能。

  在部分网络环境中，所有数据报均需携带安全选项（Security Option）。

  选项字段长度可变，可包含零个或多个选项。选项的格式分为以下两种情况：

  情况 1：仅包含 1 字节的选项类型（Option-Type）字段。

  情况 2：包含 1 字节的选项类型（Option-Type）字段、1 字节的选项长度（Option-Length）字段，以及实际的选项数据（Option-Data）字段。

  选项长度（Option-Length）字段的取值包含选项类型（Option-Type）字段、选项长度（Option-Length）字段本身，以及选项数据（Option-Data）字段的总字节数。

  选项类型（Option-Type）字段可拆分为 3 个亚字段，具体如下：

  1 比特：复制标志（Copied Flag）

  2 比特：选项类别（Option Class）

  5 比特：选项编号（Option Number）

  复制标志（Copied Flag）用于标识分片时是否将该选项复制到所有分片中：

  0 = 不复制（仅保留在原始数据报的首片）

  1 = 复制（同步到所有分片）

  选项类别（Option Class）的取值定义如下：

  0 = 控制类（Control）

  1 = 预留（供未来使用）

  2 = 调试与测量类（Debugging and Measurement）

  3 = 预留（供未来使用）[原文未完整显示，按 RFC 791 标准补全逻辑]
+ 
